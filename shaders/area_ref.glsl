...
bool IntersectTriangle(Ray ray, vec3 p0, vec3 p1, vec3 p2,
                        out float hit, out vec3 triangleNormal)
{
    vec3 e0 = p1 - p0;
    vec3 e1 = p0 - p2;
    triangleNormal = cross( e1, e0 );

    vec3 e2 = ( 1.0 / dot( triangleNormal, ray.dir ) ) * ( p0 - ray.origin );
    vec3 i  = cross( ray.dir, e2 );
    vec3 barycentricCoord = vec3(0,0,0);
...
void main()
{
    ...
    float distToFloor;vec3 triangleNormal;
    //bool hitFloor = RayPlaneIntersect(ray, floorPlane, distToFloor);
    bool hitFloor = IntersectTriangle(ray, vec3(-1,5,-1),vec3(2,5.5,0),vec3(1,5.5,1),  distToFloor, triangleNormal);
    if (hitFloor)
    {
